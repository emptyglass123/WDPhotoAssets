...
#import "WDPhotoPreviewController.h"
#import "WDAssetCell.h"
#import "WDAssetModel.h"
#import "UIView+Layout.h"
#import "WDImageManager.h"
#import "WDVideoPlayerController.h"

@interface WDPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate> {
...
@implementation WDPhotoPickerController
...
- (void)viewDidLoad {
    ...
    [[WDImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:imagePickerVc.allowPickingVideo completion:^(NSArray<WDAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    [_collectionView registerNib:[UINib nibWithNibName:@"WDAssetCell" bundle:nil] forCellWithReuseIdentifier:@"WDAssetCell"];
}
...
- (void)previewButtonClick {
    ...
    WDPhotoPreviewController *photoPreviewVc = [[WDPhotoPreviewController alloc] init];
...
- (void)okButtonClick {
    ...
    for (NSInteger i = 0; i < _selectedPhotoArr.count; i++) {
        [photos addObject:@1];[assets addObject:@1];
        [infoArr addObject:@1];
    }
    
    for (NSInteger i = 0; i < _selectedPhotoArr.count; i++) {
        WDAssetModel *model = _selectedPhotoArr[i];
        [[WDImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    WDAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"WDAssetCell" forIndexPath:indexPath];
    WDAssetModel *model = _photoArr[indexPath.row];
    cell.model = model;
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            WDImagePickerViewController *imagePickerVc = (WDImagePickerViewController *)weakSelf.navigationController;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == TZAssetModelMediaTypeVideo) {
        if (_selectedPhotoArr.count > 0) {
            ...
            WDVideoPlayerController *videoPlayerVc = [[WDVideoPlayerController alloc] init];
...
- (void)pushPhotoPrevireViewController:(WDPhotoPreviewController *)photoPreviewVc {
...
- (void)getSelectedPhotoBytes {
    ...
    [[WDImageManager manager] getPhotosBytesWithArray:_selectedPhotoArr completion:^(NSString *totalBytes) {
...
- (void)resetCachedAssets {
    ...
    [[WDImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[WDImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                            targetSize:AssetGridThumbnailSize
                                           contentMode:PHImageContentModeAspectFill
                                               options:nil];
        [[WDImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                           targetSize:AssetGridThumbnailSize
                                          contentMode:PHImageContentModeAspectFill
                                              options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        WDAssetModel *model = _photoArr[indexPath.item];
